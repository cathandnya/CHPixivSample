...
- (id) initWithEncoding:(NSStringEncoding)enc async:(BOOL)b {
	...
	if (self) {
		self.rootTag = [[[ScrapingTag alloc] initWithDictionary:[[CHPixivConstants sharedInstance] valueForKeyPath:@"medium.scrap"]] autorelease];
	}
	return self;
}
...
- (void) endDocument {
	NSMutableDictionary *mdic = (NSMutableDictionary *)[super evalResult:[[CHPixivConstants sharedInstance] valueForKeyPath:@"medium.eval"]];
	self.info = mdic;
		
	if (!noComments && [info objectForKey:@"IllustID"] && [info objectForKey:@"UserID"]) {
		NSMutableURLRequest		*req = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:[[CHPixivConstants sharedInstance] valueForKeyPath:@"urls.comment_history"]]];
		NSString				*body;
		[req autorelease];
